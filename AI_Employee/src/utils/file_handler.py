...
class FileHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        # Check file size
        try:
            from .file_parser import is_file_too_large
        ...
        for attempt in range(max_retries):
            try:
                ...
                if task is None:
                    self.logger.info(f"File already processed: {os.path.basename(file_path)}")
    ...
    def on_modified(self, event):
        ...
        Args:
            event: FileSystemEvent from watchdog
        """
        # For Silver tier, we only care about new files, not modifications
        pass

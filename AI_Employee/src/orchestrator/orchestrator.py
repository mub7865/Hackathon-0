...
from src.orchestrator.state_manager import StateManager, OrchestratorState
from src.orchestrator.task_processor import TaskProcessor, Classification
from src.orchestrator.approval_handler import ApprovalHandler, ApprovalResult
from src.utils.task_file import TaskFile
...
class Orchestrator:
    ...
    def _process_needs_action(self) -> int:
        """
        Process tasks in Needs_Action folder using Claude Code CLI.
        ...
        for task_file in task_files:
            try:
                ...
                # Invoke Claude Code CLI to process task
                success = self._invoke_claude_code_for_task(task_file)
    ...
    def _invoke_claude_code_for_task(self, task_file: Path) -> bool:
        """
        Invoke Claude Code CLI to process a task file.

        Args:
            task_file: Path to task file in Needs_Action

        Returns:
            True if Claude Code successfully processed the task
        """
        import subprocess
        ...
        try:
            ...
            # Check if this is a social media post (content already exists, no draft needed)
            if 'type: facebook_post' in task_content or 'type: linkedin_post' in task_content or 'type: instagram_post' in task_content:
                self.logger.info(f"Social media post detected, moving directly to Pending_Approval: {task_file.name}")

                # Update status to pending_approval
                updated_content = task_content.replace('status: needs_action', 'status: pending_approval')
                ...
                self.logger.info(f"Social media post moved to Pending_Approval: {task_file.name}")
                return True

            # Check if this is an accounting task (invoice, payment, expense)
            if ('type: accounting' in task_content or 'type: invoice' in task_content or
                'type: payment' in task_content or 'type: expense' in task_content):
                self.logger.info(f"Accounting task detected, moving directly to Pending_Approval: {task_file.name}")

                # Update status to pending_approval
                updated_content = task_content.replace('status: needs_action', 'status: pending_approval')
                if 'status: pending_approval' not in updated_content:
                ...
                self.logger.info(f"Accounting task moved to Pending_Approval: {task_file.name}")
                return True

            # Check if this is a social media generation request
            if 'type: social_media' in task_content:
                self.logger.info(f"Social media generation request detected: {task_file.name}")
                return self._generate_social_media_post(task_file, task_content)

            # Create prompt for Claude Code
            prompt = f"""Process this task from the AI Employee system:
...
2. Use the appropriate Agent Skill (/process-task, /email-reply, /whatsapp-reply, /linkedin-post, /request-approval)
3. Generate a draft response
4. Move the task to vault/Pending_Approval/ with status: pending_approval
5. Add your draft in the "## Draft Response" section

Execute the task now."""

            # Invoke Claude Code CLI using ccr code
            # Build command as string for shell=True on Windows
            cmd = f'ccr code --print --dangerously-skip-permissions --output-format text "{prompt}"'

            # Remove CLAUDECODE from environment to prevent nested session error
            import os
            env = os.environ.copy()
            env.pop('CLAUDECODE', None)  # Remove CLAUDECODE if it exists
            ...
            result = subprocess.run(
                ...
                env=env  # Use modified environment without CLAUDECODE
            )

            if result.returncode == 0:
                # Check if task was moved to Pending_Approval
                pending_file = self.pending_approval / task_file.name
                if pending_file.exists():
                    self.logger.info(f"Task moved to Pending_Approval: {task_file.name}")
                    return True
                else:
                    self.logger.warning(f"Claude Code ran but task not in Pending_Approval: {task_file.name}")
                    return False
            else:
            ...
            self.logger.error(f"Claude Code timeout processing {task_file.name}")
            return False
        except Exception as e:
            self.logger.error(f"Error invoking Claude Code: {e}")
            return False

    def _generate_social_media_post(self, task_file: Path, task_content: str) -> bool:
        """
        Generate social media post content using Anthropic API directly
        ...
        try:
            ...
            if len(parts) < 3:
                self.logger.error("Invalid task file format")
                return False
            ...
            image_path = frontmatter.get('image_path')  # Extract image path if provided
            ...
            if '## Tone' in body:
                tone_section = body.split('## Tone')[1].split('##')[0].strip()
                tone = tone_section

            if '## Target Audience' in body:
                audience_section = body.split('## Target Audience')[1].split('##')[0].strip()
                target_audience = audience_section

            # Build prompt for Claude
            prompt = f"""Generate a {platform} post based on these requirements:

{requirements}

Please generate engaging {platform} post content that:
- Follows the requirements exactly
- Uses appropriate emojis for engagement
- Includes relevant hashtags
- Has a clear call-to-action or question at the end
- Is optimized for {platform}'s format and audience
...
Return ONLY the post content, no explanations or meta-commentary."""
            ...
            self.logger.info(f"Generated {platform} post content ({len(generated_content)} chars)")

            # Create new task file with generated content
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            new_filename = f"{platform}_post_{timestamp}.md"
            ...
            # Add image_path if provided (required for Instagram)
            if image_path:
                new_frontmatter['image_path'] = image_path

            # Build new file content
            new_content = "---\n"
            new_content += yaml.dump(new_frontmatter, default_flow_style=False, sort_keys=False)
            new_content += "---\n\n"
            new_content += "## Original Message\n"
            new_content += f"# {platform.title()} Post - AI Generated\n\n"
            new_content += "## Post Content\n\n"
            new_content += generated_content + "\n\n"
            new_content += "## Classification\n"
            new_content += "**SENSITIVE**: Public social media posting requires human approval before publication.\n"

            # Write to Pending_Approval
            pending_file = self.pending_approval / new_filename
            pending_file.write_text(new_content, encoding='utf-8')

            # Remove original task file
            task_file.unlink()

            self.logger.info(f"Generated post moved to Pending_Approval: {new_filename}")
            return True

        except Exception as e:
            self.logger.error(f"Error generating social media post: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def _execute_approved_task_direct(self, task_file: Path) -> bool:
        ...
        try:
            ...
            # Skip draft extraction for accounting and social media tasks (they don't need draft content)
            if task_type in ['invoice', 'payment', 'expense', 'facebook_post', 'instagram_post']:
                # These tasks go directly to execution
                pass
            else:
                # Extract draft based on task type (for communication tasks)
                if task_type == 'email':
                    ...
                    # Extract from "## Draft Response" section (same as email/whatsapp)
                    if "## Draft Response" in body:
                        draft_section = body.split("## Draft Response")[1]
                        if "## Classification" in draft_section:
                            draft_content = draft_section.split("## Classification")[0].strip()
                            ...
                            draft_content = draft_section.strip()
                    # Fallback: Try old format "### Post Content:"
                    elif "### Post Content:" in body:
                ...
                # Validate draft content for communication tasks only
            ...
            elif task_type in ['facebook_post', 'instagram_post']:
                # Gold Tier: Social Media posts (Facebook/Instagram)
                from actions.social_media_actions import process_approved_post

                self.logger.info(f"Processing {task_type}")

                # Process social media post (function handles everything including moving to Done)
                status = process_approved_post(str(task_file))

                if status == 'done':
                    # Task already moved to Done by social_media_actions
                    self.logger.info(f"Social media post completed: {task_file.name}")
                    return True
                else:
                    self.logger.error(f"Social media post failed: {task_file.name}")
                    return False

            elif task_type in ['invoice', 'payment', 'expense']:
                # Gold Tier: Accounting actions (Odoo integration)
                from actions.accounting_actions import process_approved_transaction

                self.logger.info(f"Processing {task_type}: {frontmatter.get('customer') or frontmatter.get('vendor')}")

                # Process transaction in Odoo (function handles everything including moving to Done)
                status = process_approved_transaction(str(task_file))

                if status == 'done':
                    # Task already moved to Done by accounting_actions
                    self.logger.info(f"Accounting transaction completed: {task_file.name}")
                    return True
                else:
                    self.logger.error(f"Accounting transaction failed: {task_file.name}")
                    return False
